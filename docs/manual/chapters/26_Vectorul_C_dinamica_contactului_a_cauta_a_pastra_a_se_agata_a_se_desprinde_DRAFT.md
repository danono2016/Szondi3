...
După `d` și `m`, tentația cea mai firească ar fi să credem că știm deja vectorul C.

`d` ne-a spus dacă mișcarea contactului se orientează spre un obiect nou sau persistă la cel vechi. `m` ne-a spus dacă legătura caută sprijin și securizare sau se mișcă spre desprindere și libertate.

Am avea, așadar, două axe și ar rămâne doar să le punem una lângă alta.
...
Szondi numește această dinamică **Kontakttrieb**, vectorul C:
...
Cea mai bună intrare în această logică este una dintre combinațiile care contrazic intuiția obișnuită:

`C(+,+)`.

La `+d`, căutarea noului este activă. La `+m`, agățarea și securizarea legăturii sunt active.

Dacă am confunda factorii, combinația ar părea imposibilă. Cum să cauți noul și, în același timp, să te agăți?

Pentru Szondi, tocmai aceasta este una dintre posibilitățile contactului. În *Lehrbuch*, configurația este legată de forme de legătură bilaterală sau multilaterală: câmpul se deschide către alte obiecte fără ca funcția de agățare să dispară.

Aici se vede de ce `+d` nu înseamnă libertate față de obiect și de ce `+m` nu înseamnă fidelitate față de unul singur.

Căutarea răspunde la întrebarea: **spre ce obiect se îndreaptă contactul?**

Agățarea răspunde la alta: **cum este securizată legătura cu obiectul?**

Cele două răspunsuri pot coincide. Dar nu sunt obligate s-o facă.

Aceasta este prima regulă adevărat vectorială a contactului: **partenerul factorial nu adaugă doar informație; schimbă situația funcțională a reacției.**

## Când ambele axe țin spre vechi

O combinație mai ușor de intuit este:

`C(−,+)`.

`−d` păstrează orientarea către vechi. `+m` activează agățarea și securizarea.

Aici ambele mișcări converg asupra obiectului deja existent. În vocabularul lui Szondi apar `Kleben`, `Sich-Anklammern`, `Treue` și, în anumite contexte istorice, `Inzestbindung`.

Dar nici această convergență nu trebuie comprimată într-o etichetă de persoană.

`C(−,+)` nu înseamnă „o persoană fidelă” și nici „o persoană dependentă”. Înseamnă că, în acea imagine vectorială, tendința de a persista la vechi și tendința de a securiza legătura sunt simultan în prim-plan.
...
Până acum am urmărit configurații în care fiecare factor avea o direcție simplă. Dar fiecare dintre cele două probleme poate deveni ambivalentă în sine.

La `C(±,0)`, conflictul este în `d`: căutarea și persistența sunt simultan active, în timp ce `m` nu ocupă prim-planul într-o direcție simplă.

La `C(0,±)`, conflictul este în `m`: agățarea și desprinderea sunt simultan active, în timp ce `d` este nul.

Szondi numește aceste forme `Zwangskontakt`.

Mai important decât numele este însă localizarea dilemei.

În prima configurație, întrebarea este: **nou sau vechi?**

În a doua: **legare sau desprindere?**

Ambele pot produce ezitare, blocaj și tensiune. Dar nu sunt aceeași ezitare.
...
Ar fi ușor să-l traducem prin „lipsă de contact”. Szondi folosește într-adevăr termeni precum `Kontaktlosigkeit` și `Desintegration des Kontakttriebes`.

Dar nu se oprește aici.

În *Lehrbuch* și, mai târziu, în scrierile terapeutice, aceeași configurație poate apărea într-o situație aproape opusă: contactul exterior nu este necesar fiindcă relația duală cu obiectul matern sau substitutul lui ocupă întregul câmp. Szondi vorbește despre `Schoßkindkontakt`.
...
Așadar, `C(0,0)` nu înseamnă „persoana nu are relații” și nici „nu are contact cu realitatea”. Spune numai că niciuna dintre cele patru tendințe nu apare în prim-plan în forma obișnuită. De ce se întâmplă asta rămâne o întrebare deschisă până când vedem contextul.

Zero-ul dublu nu este o concluzie. Este o cerere de explicație.

## De la tabel la mișcare
...
Mai întâi obiectul este căutat.

Apoi, odată găsit, contactul persistă la el.
...
Dacă una devine unilaterală, dacă două intră în conflict sau dacă întregul circuit se blochează, Vektorbild-ul surprinde o anumită organizare a acestei mișcări.

De aceea `C` este mai bine înțeles ca dinamică decât ca listă de semnificații.

## Contactul nu este întreaga relație cu celălalt

Tocmai pentru că vocabularul vectorului C este atât de apropiat de limbajul relațiilor, există riscul să-i dăm mai mult decât spune.

Szondi insistă în scrierile târzii că **Liebesfähigkeit** și **Kontaktfähigkeit** nu sunt sinonime.

Iubirea aparține propriei drame pulsionale. Participarea și proiecția aparțin problemei Eului. Contactul descrie altceva: raportul cu obiectele prin căutare, persistență, agățare și desprindere.

Aceste registre se pot împleti în viață. Nu devin prin asta identice în sistem.

De aceea vectorul C nu poate fi tradus în bloc prin „atașament”. O asemenea traducere ar șterge diferențele dintre `d` și `m`, ar transforma o doctrină istorică într-o teorie contemporană și ar face invizibil tocmai ceea ce testul încearcă să separe.
...
Dar o persoană nu trăiește în patru vectori separați.

În același moment poate iubi și respinge, poate acumula afect și îl poate controla, poate revendica să fie și poate spune Nu, poate căuta un obiect și se poate agăța de altul.

Cele opt radicale nu au, prin ele însele, un centru comun de comandă.

Aici apare o dificultate pe care niciun capitol factorial și niciun capitol vectorial nu o mai poate rezolva.

Dacă viața pulsională este construită din asemenea contrarii, trebuie să existe o problemă a coordonării lor.

La Szondi, această problemă se numește **Eu**.