...
class TriebklasseHealthStatusLimitTests(unittest.TestCase):
    def test_doctrine_preserves_explicit_health_status_limit_and_counterexamples(self):
        ...
        self.assertIsNone(anchor["pdfPath"])
        self.assertIsNone(anchor["printedPage"])
        self.assertIsNone(anchor["visualArbitrationNote"])
        self.assertIn("ieelticl krank oder noch gesund", doctrine["sourceExcerpt"])
        self.assertEqual(doctrine["assertionMode"], ["METHOD_RULE"])
        self.assertTrue(doctrine["ambiguities"])
        self.assertIn("no PDF", doctrine["ambiguities"][0])
