...
<w:document xmlns:w="{ca.W}" xmlns:r="{ca.R}" xmlns:mc="{ca.MC}" xmlns:a="{ca.A}" xmlns:wp="{ca.WP}">
...
class CanonicalAccessRegressionTests(unittest.TestCase):
...
<w:p><w:r><w:t>Entry two</w:t><w:fldChar w:fldCharType="end"/></w:r></w:p>'''
        records, _ = ca.extract_docx_bytes(
    ...
    def test_table_property_exceptions_are_preserved_as_structural_metadata(self):
        ...
        self.assertEqual(meta["tag"], ca.qn(ca.W, "tblPrEx"))
        self.assertEqual(meta["children"][0]["tag"], ca.qn(ca.W, "tblW"))
        self.assertEqual(meta["children"][0]["attributes"][ca.qn(ca.W, "w")], "5000")
