...
def _profile(**overrides):
    return build_profile(
        _reaction(factor, overrides.get(factor, "0")) for factor in FACTORS
    )


def _case_a():
    # Reference Case A: h- s+ | e+ hy0 | k+ p+ | d- m-
...
def _relation_id(nucleus, anchor_claim_id: str) -> str:
    matches = tuple(
        relation.relation_id
        for relation in nucleus.relations
        if relation.anchor_claim_id == anchor_claim_id
    )
    if len(matches) != 1:
        raise AssertionError(
            f"Expected exactly one relation for {anchor_claim_id}, got {matches}"
        )
    return matches[0]
...
def _case_a_writer_fixture(projection, plan):
    ...
    r42 = _relation_id(n1, "IC_SZONDI_PRIMARY_000042")
    r73 = _relation_id(n1, "IC_SZONDI_PRIMARY_000073")
    r38 = _relation_id(n2, "IC_SZONDI_PRIMARY_000038")
    r78 = _relation_id(n3, "IC_SZONDI_PRIMARY_000078")
        ...
        "items": [
            {
                ...
                "text": (
                    "Materialul se organizează în trei direcții distincte. Ele pot fi "
                    "citite împreună ca hartă de explorare, dar rămân separate la "
                    "nivelul relațiilor pe care datele le autorizează."
                ),
            },
            {
                "item_id": "A-002",
                ...
                "text": (
                    ...
                    "Introiecția și expansiunea Eului sunt articulate aici prin relația "
                    "autorizată a configurației, nu doar prin simpla coexistență."
                ),
            },
            {
                "item_id": "A-003",
                ...
                "speech_act": EXPLANATION,
                "text": (
                    "Aceeași configurație deschide problema Allessein (aspirația de a "
                    "fi totul), a Deflation (reducerea acestei pretenții) și a Eului care "
                    "ia poziție față de realitate. Formula nu stabilește dacă această "
                    "limitare reușește sau eșuează în cazul persoanei."
                ),
            },
            {
                "item_id": "A-004",
                ...
                "speech_act": QUESTION,
                "text": (
                    "Ce se întâmplă, în situații concrete, când o investiție, un rol sau "
                    "o posibilitate importantă întâlnește o limită impusă de realitate?"
                ),
            },
            {
                "item_id": "A-005",
                ...
                "text": (
                    ...
                    "identificare. Identificarea nu este identitate și nu definește "
                    "persoana în întregime."
                ),
            },
            {
                "item_id": "A-006",
            ...
            },
            {
                ...
                "text": (
                    ...
                    "de protecție a Eului. Aceasta nu autorizează un diagnostic narcisic "
                    "și nu selectează ca fapt personal ramurile sexuale istorice din "
                    "contextul doctrinei."
                ),
            },
            {
                ...
                "speech_act": QUESTION,
                "text": (
                    "Ce precede, dacă apare clinic, închiderea contactului și ce se "
                    "schimbă în raport cu interlocutorul?"
                ),
            },
            {
                "item_id": "A-009",
                ...
                "text": (
                    ...
                    "autorizează o singură dinamică psihodinamică globală care să le "
                    "explice pe toate."
                ),
            },
        ]
...
class ClinicalNarrativePlanTests(unittest.TestCase):
    def test_case_a_plan_is_minimal_editorial_plus_three_nuclei_plus_closing(self):
        ...
        n2 = profile.nucleus_for_claim("IC_SZONDI_PRIMARY_000038")
        n3 = profile.nucleus_for_claim("IC_SZONDI_PRIMARY_000078")
        for nucleus in (n1, n2, n3):
            section = plan.section_for_nucleus(nucleus.nucleus_id)
            self.assertEqual(section.allowed_claim_ids, nucleus.support_claim_ids)
            self.assertEqual(section.allowed_relation_ids, nucleus.relation_ids)
        ...
        self.assertEqual(
            no_bridge_pairs,
            {
                frozenset((n1.nucleus_id, n2.nucleus_id)),
                frozenset((n1.nucleus_id, n3.nucleus_id)),
                frozenset((n2.nucleus_id, n3.nucleus_id)),
            },
        )

    def test_null_synthesis_preserves_editorial_synthesis_without_bridge_authority(self):
        _, _, plan = _case_a()
        self.assertEqual(plan.global_synthesis_mode, NULL_SYNTHESIS)
        editorial = tuple(
...
class ClinicalNucleusWriterPOCTests(unittest.TestCase):
    def test_case_a_replay_binds_every_item_to_scope_nucleus_claims_and_relations(self):
        _, projection, plan = _case_a()
        result = replay_clinical_nucleus_writer_response(
        ...
        actual_orders = [section_order[item.section_id] for item in result.items]
        self.assertEqual(actual_orders, sorted(actual_orders))
        ...
        self.assertIn("rămân distincte", result.items[-1].text)
        self.assertIn("nu autorizează", result.items[-1].text)

    def test_writer_rejects_cross_nucleus_and_editorial_relation_consumption(self):
        _, projection, plan = _case_a()
        profile = projection.profile(1)
        n1 = profile.nucleus_for_claim("IC_SZONDI_PRIMARY_000042")
        n2 = profile.nucleus_for_claim("IC_SZONDI_PRIMARY_000038")
        ...
        n2_item = next(
            ...
            if item["nucleus_id"] == n2.nucleus_id
            and item["speech_act"] == EXPLANATION
        )
    ...
    def test_writer_rejects_nonexistent_nucleus_and_missing_plan_section(self):
        ...
        decoded = json.loads(response["output_text"])
        closing_id = next(
            item.section_id for item in plan.sections if item.purpose == CLOSING
        )
        decoded["items"] = [
    ...
    def test_request_is_offline_build_only_and_style_is_separate_for_ablation(self):
        _, projection, plan = _case_a()
        ...
        self.assertIn("NULL_SYNTHESIS", request["instructions"])
        payload = request["input"][0]["content"][0]["text"]
        self.assertIn('"narrative_plan"', payload)
        self.assertIn('"profile_projection"', payload)
