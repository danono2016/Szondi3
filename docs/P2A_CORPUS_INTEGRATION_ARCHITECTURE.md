...
P2A is not one book summary and not conversational memory. It is a corpus-scale doctrinal knowledge base populated source by source and later connected transversally.

The durable pipeline is:

`ADMITTED SOURCE -> CANONICAL U###### STREAM -> SOURCE-ORDER REVIEW -> SELECTIVE CRITICAL DOCTRINE -> COVERAGE LEDGER -> CONCEPT INDEX -> CROSS-SOURCE RELATIONS -> RETRIEVAL -> CANONICAL RECONSULTATION`

The canonical source is the complete textual memory. The doctrine registry is a compact semantic/provenance map containing only critical, distinct, future-retrieval-worthy doctrine. Coverage proves review; it does not require a doctrine object for every passage.

The selection/compaction policy is normative for working population:

`docs/P2A_DOCTRINE_SELECTION_AND_COMPACTION_POLICY.md`

The transversal-layer contract is:
...
## 3. Source ownership and concurrency

Multiple contributors may populate P2A in parallel if they obey source ownership and stable identity rules.

A contributor working on a source must:

1. verify current repository/PR state before writing;
2. work through the normal branch/PR process;
3. preserve the source's `sourceId` and local doctrine-ID sequence;
4. review/extract in source order with bounded batches;
...
7. never renumber doctrine IDs already committed or reserved;
8. never edit another source's entries merely to harmonize terminology;
9. record cross-source parallels as relation candidates rather than rewriting either entry.
...
Every source should eventually have three durable classes of artifact.

### 4.1 Selective critical registry
...
Historical reading notes may remain in Git history or non-authoritative archival paths, but they are not required continuity artifacts and do not govern current work.

## 5. Cross-source conceptual layer

Source-order extraction alone is necessary but insufficient for later expert retrieval. After compact critical entries exist, a separate transversal index connects doctrine without changing it.
...
The integration discipline, relation record contract and open-question contract are defined in `docs/P2A_TRANSVERSAL_INDEX_AND_RELATION_CONTRACT.md`.
...
Actual cross-source relations should normally be committed only after all referenced doctrine IDs exist stably and both canonical contexts have been reconsulted.

## 8. Split-work integration

Independent source-local work on volumes such as Ich-Analyse A/B contributes without restructuring another source's registry.

Expected durable outputs are:

1. selective critical source-order doctrine batches;
2. complete or explicitly bounded coverage and verification for each sourceId;
3. high-value cross-source relation candidates;
4. explicit unresolved translation/terminology questions;
5. no renumbering or rewriting of other source-local entries.

No provisional executable rule should be justified by the mere expectation that a later source will support it. P2B may use only already represented doctrine with sufficient source support.

## 9. Retrieval contract

A doctrinal query should:
...
6. consult PDF where visual/layout/formula evidence affects meaning;
7. synthesize only afterward.

This is why the registry should remain compact: retrieval returns to the canonical source rather than replacing it.

## 10. Merge and concurrency safety

One active writer per source-local doctrine-ID sequence is preferred; independent sources may proceed concurrently. Shared schema/index/relation policy changes are integration-sensitive. A source PR should not casually change shared ontology to fit one book. Cross-source relations should normally be committed only after referenced doctrine IDs exist stably.

## 11. Quality gates before corpus-scale completion

P2A corpus completion requires:

- exhaustive or explicitly bounded source-order coverage ledgers;
...
This architecture does not contain a mutable project plan, source-processing checkpoint, chat assignment or implementation resume point.

Current branch/PR state, live source coverage, active executable frontier and next authorized development action are reconstructed from the repository and may be summarized in `docs/PROJECT_STATE.md`.

P2A work proceeds only where a concrete source or integration task requires it. P2B work may proceed independently when its doctrine dependencies are already stable and source-grounded; unfinished corpus coverage is not a blanket blocker on unrelated, adequately supported executable interpretation.

## Final invariant

> **Read everything required by the concrete source task. Account for reviewed coverage. Store only what is doctrinally critical. Retrieve the rest from the canonical source when needed.**