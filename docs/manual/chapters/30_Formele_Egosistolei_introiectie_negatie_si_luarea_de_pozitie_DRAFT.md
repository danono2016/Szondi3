# Capitolul 30 — Formele Egosistolei: introiecție, negație și luarea de poziție

**Statut:** DRAFT v1  
**Notă editorială:** redactat după research 10/10 și bounded deep corpus pass. Capitolul dezvoltă formele radicalului `k`; integrarea Eului și `Pontifex oppositorum` rămân pentru cap. 31.

---

Egodiastola a arătat cum se poate lărgi Eul și cum își poate schimba locul `Seinsmacht`: prin participație, inflație, proiecție.

Egosistola pune o altă problemă.

Ce face Eul cu ceea ce îl revendică?

Szondi nu răspunde printr-un singur „nu”. Radicalul `k` poate spune și „da”.

Tocmai de aceea îl numește **`das stellungnehmende Ich`** — Eul care ia poziție.

Cele două tendințe ale sale sunt:

`+k` — **Introjektion, Einverleibung, Bejahung**;

`−k` — **Negation, Verneinung**.

Aceeași Egosistolă restrânge expansiunea în două feluri opuse: poate lua ceva în sine sau îl poate refuza.

## `Stellungnahme`: da sau nu

În *Ich-Analyse*, `k` nu este numai `Ich-Einengung`. Este partea Eului care ia poziție față de ceea ce Egodiastola aduce în `Ich-Feld` și în `Wunschbewußtsein`.

Szondi evită cuvântul „voință”, prea încărcat filozofic, și păstrează `Stellungnahme`.

A lua poziție nu înseamnă aici o hotărâre conștientă în sens obișnuit. În aparatul testologic, vorbim despre funcțiile inconștiente ale radicalului `k`.

Iar `Stellungnahme` este legată de `Wirklichkeit prüfen`, de `Realitätsprüfung`.

Eul se întoarce spre realitate și verifică. Ce poate fi acceptat? Ce trebuie respins? Ce există realmente din ceea ce proiecția sau inflația au pus în joc?

Din această verificare pot ieși două răspunsuri radical diferite.

Unul spune: **da, îl iau în mine.**

Celălalt: **nu.**

## Introiecția: Eul spune da prin `Einverleibung`

`+k` este ancora testologică a introiecției.

Szondi definește `Introjektion` printr-o serie de termeni care trebuie lăsați să lucreze:

`Inbesitznahme`.

`Einverleibung`.

`Kapitalisierung`.

A lua în posesie. A încorpora. A transforma într-un bun al Eului.

Ținta radicală este `Alles-Haben`.

Această formulă poate suna materialist dacă o citim prea îngust. La Szondi, `Haben` nu se referă numai la obiecte materiale. Eul poate lua în posesie un obiect, o valoare, o reprezentare, un interes, o cunoaștere, o capacitate, o figură familială.

În loc să rămână o revendicare de `Sein`, conținutul poate deveni un `Habinteresse`.

Capitolul precedent a arătat mișcarea în forma ei cea mai limpede: nu mai trebuie să fii ceea ce revendicai să fii; poți avea acel conținut ca interes, cunoaștere sau bun al Eului.

Introiecția restrânge astfel Egodiastola fără s-o distrugă.

O **încorporează**.

În *Ich-Analyse*, Szondi urmărește introiecția în trei registre — personal, colectiv și familial. Nu avem nevoie de o taxonomie pentru a înțelege rangul funcției. Important este că `Einverleibung` poate lucra asupra unor conținuturi provenite din istorii foarte diferite, iar rezultatul intră în inventarul Eului.

De aceea introiecția este legată, în sistemul său, de identificare, caracter, obiect-idealuri și `Haben`.

## Negația: Eul spune nu

La polul opus se află `−k`.

`Negation` nu este la Szondi simplul act verbal de a nega și nici doar opoziția vizibilă față de ceilalți.

Seria lui este mai largă:

`Vermeiden` — a evita;

`Verneinen` — a nega;

`Hemmen` — a inhiba;

`Entfremden` — a înstrăina;

`Verdrängen` — a refula.

În formele extreme apare `Desimagination`: distrugerea idealurilor de `Sein` și `Haben`, până la `Destruktion`.

Aici se vede de ce negația nu poate fi tradusă pur și simplu prin negativism.

Negativismul este una dintre formele istorice extreme. Radicalul este mai larg: Eul poate restrânge prin refuz, renunțare, inhibare, evitare sau refulare.

În *Ich-Analyse I*, Szondi insistă că direcția originară a Egosistolei este spre interior: negația limitează expansiunea lumii interioare. Numai secundar sistola se poate întinde asupra raportului cu lumea exterioară.

Prin `Realitätsprüfung`, `−k` poate spune nu unei revendicări tocmai pentru că o confruntă cu ceea ce este posibil, valoros sau real.

## Două tendințe opuse, același radical

Introiecția și negația par, la prima vedere, să facă lucruri incompatibile.

Prima acceptă.

A doua respinge.

Szondi le numește însă un **`funktionelles Gegensatzpaar`**.

Ambele aparțin Egosistolei.

`+k` restrânge prin `Einverleibung`: ia în Eu ceea ce poate fi posedat, transformat în `Haben`.

`−k` restrânge prin `Verneinung`: refuză, inhibă sau îndepărtează ceea ce nu este acceptat.

Egosistola nu este deci un „nu” opus unui „da” egodiastolic.

Ea conține propriul da și propriul nu.

Aceasta schimbă rangul lui `k`. Radicalul restrângerii își poartă contrariul în interior.

## `±k`: când da și nu sunt active împreună

Dacă `+k` și `−k` sunt tendințe opuse ale aceluiași radical, ele pot deveni simultan active.

Ancora testologică este `±k`.

În teoria Eului, Szondi numește această bifuncție **`Intronegation`**: introiecție și negație în același timp.

Eul spune da și nu.

Nu ca medie și nu ca soluție de compromis, ci ca `binnenfaktorielle Ich-Dialektik` a radicalului `k`.

Uneori aceeași revendicare este simultan acceptată și refuzată. Alteori, dintr-o pereche de tendințe contrarii, una este introiectată, cealaltă negată.

Szondi leagă istoric această bifuncție de `Zwang` și de anumite formule ale `Dur-Ich`.

Dar `±k` nu înseamnă automat nevroză obsesională.

Semnul testologic arată coactivarea tendințelor. Sensul ei depinde de configurația Sch, de restul profilului și, pentru orice afirmație clinică, de verificarea clinică și anamnestică.

## `0k`: când luarea de poziție lipsește din prim-plan

Mai rămâne o posibilitate.

Nici da, nici nu.

`0k`.

În *Lehrbuch*, Szondi îl descrie testologic ca abandonul **actual și relativ** al `Stellungnahme` în prim-plan, împreună cu absența `Realitätsprüfung` în sensul funcției `k`.

Formularea trebuie păstrată exact la acest nivel.

Un `0k` izolat nu autorizează verdictul „Eu slab”.

Szondi însuși face afirmații mai puternice numai atunci când reacția rămâne staționară în serie și intră în constelații pe care le interpretează ca tulburări ale Egosistolei.

Reacția zero spune mai întâi că funcția de luare de poziție nu ocupă prim-planul în acea configurație.

Iar absența ei schimbă imediat relația cu `p`: proiecția, inflația sau coactivarea lor pot rămâne fără contra-mișcarea obișnuită a lui `k`.

## `k` nu este simplul „nu”

Egosistola are, așadar, mai multe moduri de a limita expansiunea.

Poate încorpora: `Introjektion`.

Poate nega: `Negation`.

Le poate activa împreună: `Intronegation`.

Sau `Stellungnahme` poate lipsi din prim-plan: `0k`.

Aceste variații nu sunt patru personalități și nici patru diagnostice. Ele sunt forme testologice și funcționale ale radicalului `k`.

Mai important însă este altceva.

În `p`, participația/proiecția și inflația formează propria opoziție internă.

În `k`, introiecția și negația fac același lucru.

Iar între `p` și `k` există, la rândul ei, opoziția Egodiastolă–Egosistolă.

Eul szondian începe astfel să arate mai puțin ca o funcție unică și mai mult ca un câmp în care contrariile trebuie să poată coexista fără să se anuleze.

Aici problema nu mai este ce face o funcție.

Este cum poate rămâne Eul unul singur când funcțiile sale trag în direcții opuse.