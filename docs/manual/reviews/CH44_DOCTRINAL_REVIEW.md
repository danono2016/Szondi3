...
**Statut final doctrinar:** DOCTRINAL PASS / SCIENTIFIC AUDIT CLOSED / STYLE PASS / READER PASS CLOSED / CHAPTER STABLE / CH44-AUDIT-01 CONFIRMED/CLOSED / CH44-AUDIT-02 CONFIRMED/CLOSED / CH44-ROUND-01 SOURCE/PROCEDURE HOLD ACTIVE  
**Ramură:** `manual`

---

## Rezultatul auditului extern pe DRAFT v1

Auditul extern a reconstruit independent mecanica și a controlat vizual pasajele canonice relevante din `Lehrbuch` și cazurile numerice din `Triebpathologie I–II`.

Nucleul tehnic a fost confirmat:

- `TspQu = Σ0 / Σ±`;
- `% Sy-Re = ((Σ0 + Σ±) × 100) / (8 × N)`;
- `TspQu` descrie raportul intern dintre `0` și `±`, iar `% Sy-Re` masa totală `0+±` în toate reacțiile factoriale;
- pentru `% Sy-Re`, denominatorul real este `8×N` (`64` la 8 profile, `56` la 7 etc.);
- `20–30%` rămâne reper istoric/provizoriu formulat `vorderhand`;
- `Tabelle 13` nu este importată în calculul acestor doi indici;
- `31/0=∞` este explicit canonic;
- valorile numerice controlate mecanic și vizual trec fără eroare.

## CH44-AUDIT-02 — CONFIRMED/CLOSED

Auditul extern confirmă:

- `% Sy-Re = 20–30%` este tratat ca reper empiric istoric/provizoriu, nu normă psihometrică contemporană;
- denominatorul `8×N` este păstrat pentru lungimea reală a seriei;
- `Tabelle 13` nu este folosită fără bază canonică;
- sursa spune explicit că acești indici nu sunt suficienți singuri pentru o diagnoză clinică.

## MATERIAL ADDITION — integrare în DRAFT v2
