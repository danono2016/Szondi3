...
class TriebklasseHealthStatusLimitTests(unittest.TestCase):
    def test_doctrine_preserves_explicit_health_status_limit_and_pdf_arbitration(self):
        path = (
        ...
        self.assertIn("seelisch krank oder noch gesund", anchor["visualArbitrationNote"])
        self.assertIn("ieelticl krank oder noch gesund", doctrine["sourceExcerpt"])
        self.assertEqual(doctrine["assertionMode"], ["METHOD_RULE"])
        self.assertEqual(doctrine["ambiguities"], [])
        ...
        self.assertIn("seelisch", doctrine["reviewNotes"][1])
