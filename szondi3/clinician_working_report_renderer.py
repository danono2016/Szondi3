...
It preserves the report's wording, provenance, unresolved states, longitudinal
structural differences, clinician-authored context, and release metadata without
recalculating P1, activating P2B claims, adding clinical meaning, or using AI.
"""
...
def _finding_block(finding: Any) -> str:
    ...
    source_strength = (
        f'<p class="source-strength"><strong>Forța sursei:</strong> '
        f"{_text(finding.source_strength_note)}</p>"
    ...
    return (
        ...
        f'<div class="meta">{_text(" · ".join(meta))}</div>'
        f"<p>{_text(finding.statement)}</p>"
        f"{source_strength}{anti}"
        "</article>"
    )
...
def _render_profile_observations(report: ClinicianWorkingReport) -> str:
    ...
    for profile in report.formal.observations:
        factor_rows = "".join(
            "<tr>"
            f"<td>{_text(item.factor)}</td>"
            f"<td>{_text(item.symbol)}</td>"
            f"<td>{item.sympathetic}</td>"
            f"<td>{item.unsympathetic}</td>"
            f"<td>{item.quantum_level}</td>"
            f"<td>{_text(item.forced_null)}</td>"
            "</tr>"
            for item in profile.factors
        )
        vector_rows = "".join(
            "<tr>"
            f"<td>{_text(item.vector)}</td>"
            f"<td>{_text(_join(item.symbols, ' '))}</td>"
            "</tr>"
        ...
        blocks.append(
            ...
            '<div class="grid">'
            '<div><table><thead><tr><th>Factor</th><th>Reacție</th><th>+</th><th>-</th>'
            ...
            f"<tbody>{vector_rows}</tbody></table></div>"
            "</div>"
        )
...
def _render_series_morphology(report: ClinicianWorkingReport) -> str:
    factor_rows = "".join(
        ...
        f"<td>{_text(item.factor)}</td>"
        f"<td>{_text(_join(item.symbols, ' '))}</td>"
        f"<td>{_text(_join(item.base_symbols, ' '))}</td>"
        f"<td>{item.positive_count}</td>"
        f"<td>{item.negative_count}</td>"
        f"<td>{item.null_count}</td>"
        f"<td>{item.ambivalent_count}</td>"
        f"<td>{item.forced_null_count}</td>"
        f"<td>{_text(_join(item.tensioned_profiles))}</td>"
        f"<td>{item.quantum_total}</td>"
        "</tr>"
    ...
    vector_rows = "".join(
        "<tr>"
        f"<td>{_text(item.vector)}</td>"
        f"<td>{_text(_join(item.factors, ' / '))}</td>"
    ...
    return (
        '<h3>Factori în serie</h3>'
        '<table><thead><tr><th>Factor</th><th>Secvență</th><th>Secvență bază</th>'
        '<th>+</th><th>-</th><th>0</th><th>±</th><th>Null forțat</th>'
        '<th>Profile tensionate</th><th>Quantum total</th></tr></thead>'
        f"<tbody>{factor_rows}</tbody></table>"
        '<h3>Vectori în serie</h3>'
        '<table><thead><tr><th>Vector</th><th>Factori</th><th>Secvență</th>'
        '<th>Secvență bază</th><th>Frecvențe configurații</th></tr></thead>'
...
def _render_calculations(report: ClinicianWorkingReport) -> str:
    rows = "".join(
        ...
        f"<td>{_text(item.state)}</td>"
        f"<td>{_text(item.value)}</td>"
        f"<td>{_text(item.note)}</td>"
        "</tr>"
    ...
    return (
        '<table><thead><tr><th>Calcul</th><th>Stare</th><th>Valoare</th><th>Notă</th>'
...
def _render_status(report: ClinicianWorkingReport) -> str:
    ...
    suppressed_rows = "".join(
        ...
        f"<td>{_text(item.claim_id)}</td>"
        f"<td>{_text(item.activation_status)}</td>"
        f"<td>{_text(_join(item.missing_facts))}</td>"
        f"<td>{_text(_join(item.missing_context))}</td>"
        f"<td>{_text(_join(item.qualifications, ' | '))}</td>"
        "</tr>"
        for item in report.status.suppressed
    )
    suppressed = (
        '<details><summary>Claim-uri neactivate păstrate pentru audit '
...
def _render_experimental_complement(report: ClinicianWorkingReport) -> str:
    ...
    for item in section.evidence:
        ...
        fact_rows = "".join(
            "<tr>"
            f"<td>{_text(fact.key)}</td>"
            f"<td>{_text(fact.value)}</td>"
            f"<td>{_text(fact.input_state)}</td>"
            f"<td>{_text(fact.fact_id)}</td>"
            "</tr>"
            for fact in item.facts
        )
        evidence_blocks.append(
            f"<h3>E.K.P. test {item.test_number}</h3>"
            f"<p><strong>Reacții:</strong> {_text(factor_symbols)}</p>"
            '<table><thead><tr><th>Cheie</th><th>Valoare</th><th>Stare input</th><th>Fact ID</th>'
            f"</tr></thead><tbody>{fact_rows}</tbody></table>"
        )
...
def _render_longitudinal(report: ClinicianWorkingReport) -> str:
    ...
    for comparison in report.longitudinal:
        ...
        header_rows = "".join(
            "<tr>"
            f"<td>{_text(item.label)}</td><td>{_text(item.value_a)}</td>"
            f"<td>{_text(item.value_b)}</td><td>{_text(item.note)}</td>"
            "</tr>"
            for item in comparison.header_diffs
        ...
        factor_rows = "".join(
            ...
            f"<td>{_text(_join(item.symbol_sequence_b or (), ' '))}</td>"
            "</tr>"
        ...
        vector_rows = "".join(
            ...
            f"<td>{_text(' | '.join(' '.join(pair) for pair in (item.symbol_sequence_b or ())))}</td>"
            "</tr>"
        ...
        claim_rows = "".join(
            "<tr>"
            f"<td>{_text(_scope_label(item.key.scope, item.key.profile_number))}</td>"
            f"<td>{_text(item.key.claim_id)}</td>"
            f"<td>{_text(item.state_a)}</td>"
            f"<td>{_text(item.state_b)}</td>"
            "</tr>"
        ...
        if header_rows:
            blocks.append(
                '<h4>Diferențe de antet</h4><table><thead><tr><th>Câmp</th><th>A</th><th>B</th><th>Notă</th>'
                f"</tr></thead><tbody>{header_rows}</tbody></table>"
            )
        if factor_rows:
            blocks.append(
                '<h4>Diferențe factoriale</h4><table><thead><tr><th>Factor</th><th>A</th><th>B</th>'
                f"</tr></thead><tbody>{factor_rows}</tbody></table>"
            )
        if vector_rows:
            blocks.append(
                '<h4>Diferențe vectoriale</h4><table><thead><tr><th>Vector</th><th>A</th><th>B</th>'
                f"</tr></thead><tbody>{vector_rows}</tbody></table>"
            )
        if calculation_rows:
            blocks.append(
                '<h4>Diferențe de calcul</h4><table><thead><tr><th>Calcul</th><th>A</th><th>B</th><th>Notă</th>'
                f"</tr></thead><tbody>{calculation_rows}</tbody></table>"
            )
        if claim_rows:
            blocks.append(
                '<h4>Diferențe de stare P2B</h4><table><thead><tr><th>Scop</th><th>Claim</th><th>A</th><th>B</th>'
                f"</tr></thead><tbody>{claim_rows}</tbody></table>"
            )
        if not any((header_rows, factor_rows, vector_rows, calculation_rows, claim_rows, issues)):
...
def _render_clinician_input(report: ClinicianWorkingReport) -> str:
    ...
    synthesis = (
        ...
        f"<p>{_text(report.clinician_synthesis.text)}</p>"
        "</article>"
...
def _render_technical(report: ClinicianWorkingReport) -> str:
    ...
    return (
        '<details><summary>Manifest de release</summary><table class="kv">'
        f"{release_rows}</table></details>"
        '<details><summary>Audit structural</summary><table class="kv">'
...
def render_clinician_working_report_html(report: ClinicianWorkingReport) -> str:
    ...
    The function consumes only an already-built ``ClinicianWorkingReport``. It does
    not calculate test results, route/activate claims, alter source strength,
    synthesize longitudinal meaning, or generate clinician text.
    """
...
<section id="formal">
<h2>Structura formală actuală</h2>
{_render_profile_observations(report)}
<h3>Morfologia seriei</h3>
{_render_series_morphology(report)}
<h3>Calcule deterministe</h3>
{_render_calculations(report)}
</section>

<section id="findings">
<h2>Constatări Szondiene autorizate</h2>
{findings or '<p class="empty">Nicio constatare activă în proiecția curentă.</p>'}
</section>

<section id="boundaries">
<h2>Limite și anti-inferențe</h2>
{_render_boundaries(report)}
</section>

<section id="status">
<h2>Stări nerezolvate, blocate și neactivate</h2>
{_render_status(report)}
</section>

<section id="complement">
<h2>Complement experimental (E.K.P.)</h2>
{_render_experimental_complement(report)}
</section>

<section id="longitudinal">
<h2>Comparație longitudinală</h2>
{_render_longitudinal(report)}
</section>

<section id="clinician-input">
<h2>Integrare clinică manuală</h2>
{_render_clinician_input(report)}
</section>

<section id="provenance">
<h2>Trasabilitate doctrinară</h2>
{_render_provenance(report)}
</section>

<section id="technical">
<h2>Trasabilitate tehnică</h2>
